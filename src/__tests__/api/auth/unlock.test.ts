...
const blockedUser = { id: 5, email: 'bloqueado@gladpros.com', nomeCompleto: 'Bloqueado Test', bloqueado: true }
...
describe('POST /api/auth/unlock', () => {
  ...
  beforeEach(() => {
    ...
    require('../../../shared/lib/blocking').BlockingService.unlockWithPin.mockResolvedValue({ success: true })
    require('../../../shared/lib/blocking').BlockingService.unlockWithSecurityQuestion.mockResolvedValue({ success: true })
  ...
  it('retorna 400 quando usuário não encontrado (sem revelar enumeração)', async () => {
    ...
    ;(mockRequest.json as jest.Mock).mockResolvedValue({ method: 'pin', userId: 999, pin: '1234' })

    const response = await POST(mockRequest)
    expect(response.status).toBe(400)
    const data = await response.json()
    expect(data.error).toBe('Usuário não está bloqueado')
  })
  ...
  it('retorna 400 quando usuário não está bloqueado', async () => {
    ...
    ;(mockRequest.json as jest.Mock).mockResolvedValue({ method: 'pin', userId: 5, pin: '1234' })
  ...
  it('retorna 400 quando PIN ausente no método pin', async () => {
    ;(mockRequest.json as jest.Mock).mockResolvedValue({ method: 'pin', userId: 5 })
    const response = await POST(mockRequest)
    expect(response.status).toBe(400)
  })
  ...
  it('retorna 401 quando PIN incorreto', async () => {
    ...
    ;(mockRequest.json as jest.Mock).mockResolvedValue({ method: 'pin', userId: 5, pin: '9999' })
  ...
  it('retorna 400 quando resposta de segurança ausente', async () => {
    ;(mockRequest.json as jest.Mock).mockResolvedValue({ method: 'security', userId: 5 })
    const response = await POST(mockRequest)
    expect(response.status).toBe(400)
  })
  ...
  it('retorna 401 quando resposta de segurança incorreta', async () => {
    ...
    ;(mockRequest.json as jest.Mock).mockResolvedValue({ method: 'security', userId: 5, answer: 'errado' })
  ...
  it('retorna 200 ao desbloquear com pergunta de segurança correta', async () => {
    ;(mockRequest.json as jest.Mock).mockResolvedValue({ method: 'security', userId: 5, answer: 'correta' })
