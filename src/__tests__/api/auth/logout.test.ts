...
describe('POST /api/auth/logout', () => {
  ...
  it('limpa os três cookies de autenticação (authToken, refreshToken, sessionToken)', async () => {
    const response = await POST(mockRequest)
  ...
  it('retorna 200 mesmo quando revokeRefreshToken falha (degradação graciosa)', async () => {
    require('../../../lib/auth/token-service').revokeRefreshToken.mockRejectedValue(new Error('DB offline'))
    ;(mockRequest.headers.get as jest.Mock).mockImplementation((key: string) => {
  ...
  it('aceita token via Authorization: Bearer como alternativa ao cookie', async () => {
    ...
    expect(require('../../../shared/lib/jwt').verifyAuthJWT).toHaveBeenCalledWith('header-auth-token')
  })
})
