...
export function createServiceOrderCompletedSteps(
  ctx: ServiceOrderCompletedContext,
): PlaybookStep[] {
  return [
    {
      ...
      async run() {
        ...
        if (so.status !== 'COMPLETED') {
          throw new Error(
            `Service Order ${so.ticketNumber} não está com status COMPLETED (atual: ${so.status})`,
          );
        }
      },
    },
    {
      ...
      async run() {
        ...
        const estimatedCost =
          so.estimatedHours && so.hourlyRate
