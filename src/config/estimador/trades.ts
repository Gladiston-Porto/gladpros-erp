...
export const TRADES: TradeConfig[] = [
...

  {
    ...
    questions: [
      ...
      {
        id: 'tanks_quantity',
        label: 'Quantas unidades de tanque serão removidas?',
        type: 'number',
        required: false,
        min: 1,
        max: 5,
        defaultValue: 1,
        placeholder: '1',
        dependsOn: { questionId: 'replacing_tank', value: true },
      },
      {
        id: 'bypass_reconfig',
        label: 'Inclui reconfiguração do bypass? (quando havia 2+ tanques em paralelo)',
        type: 'boolean',
        required: false,
        helpText: 'Necessário quando dois tanques estavam ligados em bypass — re-pipe das linhas de água.',
        dependsOn: { questionId: 'replacing_tank', value: true },
      },
      {
        id: 'sediment_filter',
        label: 'Incluir instalação de filtro de sedimento whole-house?',
        type: 'boolean',
        required: false,
        helpText: 'Recomendado para proteger o tankless de partículas e prolongar a vida útil.',
      },
    ],
  },
  ...
  // ── Water Treatment ─────────────────────────────────────────────────────────
  {
    ...
    icon: '💧',
    category: 'plumbing',
    description: 'Instalação de filtros de sedimento, carvão, water softener ou sistema RO.',
    questions: [
      {
        id: 'filter_type',
        label: 'Tipo de sistema de tratamento',
        type: 'select',
        required: true,
        options: [
          { value: 'sediment', label: 'Filtro sedimento whole-house (partículas, areia, ferrugem)' },
          { value: 'carbon',   label: 'Filtro carvão ativado whole-house (cloro, odor, gosto)' },
          { value: 'softener', label: 'Water softener (abrandador — dureza, escala calcária)' },
          { value: 'ro',       label: 'Sistema RO under-sink (osmose reversa — água pura na torneira)' },
        ],
      },
      {
        id: 'whole_house',
        label: 'Instalação na linha principal (whole-house)?',
        type: 'boolean',
        required: false,
        helpText: 'Linha principal = protege toda a casa. Não se aplica a RO (sempre under-sink).',
      },
      {
        id: 'brand_supplied',
        label: 'Equipamento fornecido pelo cliente?',
        type: 'boolean',
        required: false,
        helpText: 'Se você já tiver o equipamento, apenas cobramos a mão de obra e conexões.',
      },
    ],
  },
  ...
  // ── Painting — Interior ──────────────────────────────────────────────────────
  {
    ...
    icon: '🎨',
    category: 'remodel',
    description: 'Pintura interna de cômodos: paredes, teto, molduras. Inclui prep e 2 demãos.',
    questions: [
      {
        id: 'sqft',
        label: 'Área total do imóvel (sqft)',
        type: 'number',
        required: true,
        min: 100,
        max: 15000,
        placeholder: 'Ex: 1800',
        helpText: 'Área do piso — calculamos a área de parede automaticamente.',
      },
      {
        id: 'rooms',
        label: 'Quantos cômodos serão pintados?',
        type: 'number',
        required: true,
        min: 1,
        max: 20,
      ...
      {
        id: 'paint_quality',
        label: 'Qualidade da tinta',
        type: 'select',
        required: true,
        options: [
          { value: 'basic',   label: 'Básica — Sherwin-Williams Harmony (~$35/gal)' },
          { value: 'mid',     label: 'Intermediária — S-W Duration (~$55/gal) — Recomendado' },
          { value: 'premium', label: 'Premium — S-W Emerald (~$85/gal) — máxima durabilidade' },
        ],
      },
      {
        id: 'prep_work',
        label: 'Incluir preparação de superfície? (spackle, lixamento, primer)',
        type: 'boolean',
        required: false,
        helpText: 'Recomendado em paredes com buracos, textura ou mudança drástica de cor.',
      },
    ],
  },
  ...
  // ── Painting — Exterior ─────────────────────────────────────────────────────
  {
    ...
    icon: '🏠',
    category: 'remodel',
    description: 'Pintura da fachada externa: body, trim, soffit. Inclui prep e pressure wash.',
    questions: [
      {
        id: 'sqft_facade',
        label: 'Área da fachada exterior (sqft)',
        type: 'number',
        required: true,
        min: 200,
        max: 20000,
        placeholder: 'Ex: 2400',
        helpText: 'Área das paredes exteriores — não é a área do piso.',
      },
      {
        id: 'stories',
      ...
      {
        id: 'include_trim',
        label: 'Incluir trim/molduras/soffits em cor de destaque?',
        type: 'boolean',
        required: false,
      },
      {
        id: 'pressure_wash',
        label: 'Incluir pressure wash antes de pintar?',
        type: 'boolean',
        required: false,
        helpText: 'Recomendado — garante aderência da tinta e remove mofo/sujeira.',
        defaultValue: true,
      },
      {
        id: 'paint_quality',
        label: 'Qualidade da tinta exterior',
        type: 'select',
        required: true,
        options: [
          { value: 'basic',   label: 'Básica — S-W SuperPaint Exterior (~$45/gal)' },
          { value: 'mid',     label: 'Intermediária — S-W Duration Exterior (~$65/gal) — Recomendado' },
          { value: 'premium', label: 'Premium — S-W Emerald Exterior (~$95/gal) — resistência máxima Dallas TX' },
        ],
      },
    ],
  },

  // ── Painting — Full (Interior + Exterior) ───────────────────────────────────
  {
    id: 'painting-full',
    label: 'Pintura Completa (Interior + Exterior)',
    icon: '🖌️',
    category: 'remodel',
    description: 'Projeto completo: pintura interior + exterior. Desconto de ~8% no combinado.',
    questions: [
      {
        id: 'sqft',
        label: 'Área total interna (sqft)',
        type: 'number',
        required: true,
        min: 100,
        max: 15000,
      ...
      {
        id: 'rooms',
        label: 'Cômodos internos a pintar',
        type: 'number',
        required: true,
        min: 1,
        max: 20,
      ...
      {
        id: 'include_ceiling',
        label: 'Incluir teto interno?',
        type: 'boolean',
        required: false,
      },
      {
        id: 'include_trim',
        label: 'Incluir trim/molduras no exterior?',
        type: 'boolean',
        required: false,
      },
      {
        id: 'pressure_wash',
        label: 'Incluir pressure wash exterior?',
        type: 'boolean',
        required: false,
        defaultValue: true,
      },
      {
        id: 'prep_work',
        label: 'Incluir preparação de superfície interior? (spackle, primer)',
        type: 'boolean',
        required: false,
      },
      {
        id: 'paint_quality',
        label: 'Qualidade da tinta (aplicado para ambos)',
        type: 'select',
        required: true,
        options: [
          { value: 'basic',   label: 'Básica' },
          { value: 'mid',     label: 'Intermediária — Recomendado' },
