...
export const materialSchema = z.object({
  ...
  }).positive('Selecione uma unidade válida'),
  ...
  pontoReposicao: z
    .number({
      message: 'Ponto de reposição é obrigatório',
    })
    .nonnegative('Ponto de reposição não pode ser negativo'),
...
const movimentacaoBaseSchema = z.object({
  ...
  }).positive('Selecione um material válido'),
...
export const movimentacaoEntradaSchema = movimentacaoBaseSchema.extend({
  ...
  localizacaoDestinoId: z.number({
    message: 'Localização de destino é obrigatória',
  }).positive('Selecione uma localização válida'),
});
...
export const alocacaoEquipamentoSchema = z.object({
  ...
  }).positive('Selecione um projeto válido'),
...
export const devolucaoEquipamentoSchema = z.object({
  ...
  condicaoRetorno: z.enum(['BOM', 'REGULAR', 'RUIM'], {
    message: 'Condição de retorno é obrigatória',
  }),
  observacoes: observacoesSchema,
});
...
const compraItemSchema = z.object({
  tipoItem: z.enum(['MATERIAL', 'EQUIPAMENTO'], {
    message: 'Tipo de item é obrigatório',
  }),
