import { Suspense } from 'react'
import Link from 'next/link'
import { Button } from '@gladpros/ui/button'
import { PageHeader } from "@gladpros/ui/page-header"
import { Plus } from 'lucide-react'
import DespesaList from '@/components/financeiro/despesas/DespesaList'

export const metadata = {
  title: 'Despesas | GladPros',
  description: 'Gestão de despesas e pagamentos',
};

const ListFallback = () => (
  <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 py-12">
    <div className="h-10 w-10 animate-spin rounded-full border-2 border-brand-blue border-t-transparent" />
    <p className="mt-3 text-sm text-muted-foreground">Carregando despesas...</p>
  </div>
)

export default async function DespesasPage() {
  const empresaId = 1

  return (
    <div className="space-y-6">
      <PageHeader
        title="Despesas"
        description="Gerencie despesas recorrentes, aprovações e pagamentos."
        breadcrumbs={[
          { label: 'Dashboard', href: '/dashboard' },
          { label: 'Financeiro', href: '/dashboard/financeiro' },
          { label: 'Despesas' },
        ]}
        actions={
          <Link href="/dashboard/financeiro/despesas/novo">
            <Button size="lg">
              <Plus className="h-4 w-4" />
              Nova despesa
            </Button>
          </Link>
        }
      />

      <Suspense fallback={<ListFallback />}>
        <DespesaList empresaId={empresaId} />
      </Suspense>
    </div>
  )
}
