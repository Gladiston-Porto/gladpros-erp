...
export default async function RelatoriosOrdensServicoPage() {
  ...
  const [
    ...
    prisma.serviceOrder.count({ where: { status: "CANCELED" } }),
    prisma.serviceOrder.count({ where: { criadoEm: { gte: startOfMonth } } }),
    prisma.serviceOrder.count({
      where: { status: "COMPLETED", criadoEm: { gte: startOfMonth } },
    }),
  ...
  for (const os of porMes) {
    const key = os.criadoEm.toLocaleDateString("en-US", {
      month: "short",
      year: "numeric",
      timeZone: "America/Chicago",
    });
                ...
                <div key={s.status} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 rounded-full bg-muted h-2 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.round(
                          (s._count.id / Math.max(total, 1)) * 100
                        )}%`,
                      }}
                    />
                  </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="default">{s._count.id}</Badge>
                  </div>
                </div>
              );
            })}
            ...
            {mesesLista.map(([mes, count]) => (
              <div key={mes} className="flex flex-col items-center gap-1 flex-1">
                ...
                <div
                  ...
                  style={
                    {
                      "--bar-h": `${Math.max(6, (count / maxMes) * 64)}px`,
