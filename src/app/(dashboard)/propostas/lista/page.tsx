  ...
  useEffect(() => {
    ...
    async function loadClientes() {
      try {
        ...
        const list = Array.isArray(payload) ? payload : payload.data || payload.clients || [];
        setClientes(
  ...
  const handleSend = async (id: string) => {
    try {
      ...
      setPropostas((current) =>
        current.map((item) => (item.id === id ? { ...item, status: 'ENVIADA' } : item)),
      );
    } catch {
    ...
    return (
      <Card className="border-none shadow-sm">
        <CardContent className="py-12 text-center">
          <h2 className="text-lg font-semibold text-destructive">Erro ao carregar propostas</h2>
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="default"
              onClick={handleExpirar}
              title="Cancelar propostas enviadas com validade vencida"
            >
