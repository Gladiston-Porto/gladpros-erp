...
async function PayablesContent({ empresaId }: { empresaId: number }) {
  const now = new Date();

  const [vencidas, aVencer7d, pendentes, expenses] = await Promise.all([
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className={`px-6 py-3 ${isVencida ? "text-destructive font-semibold" : "text-muted-foreground"}`}>
                        {new Date(e.dataVencimento).toLocaleDateString("en-US")}
                        {isVencida && " ⚠"}
                      </td>
  ...
  return (
    <div className="space-y-6">
      ...
      <Suspense fallback={<div className="animate-pulse h-64 rounded-2xl bg-muted" />}>
        <PayablesContent empresaId={empresaId} />
      </Suspense>
    </div>
  );
}
