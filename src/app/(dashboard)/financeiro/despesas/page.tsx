...
async function DespesasContent({ empresaId }: { empresaId: number }) {
  const now = new Date();
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59);

  const [despesasMes, despesasPendentes, despesasVencidas, despesas] = await Promise.all([
              ...
              <tbody className="divide-y divide-border">
                {despesas.map((d) => (
                  <tr key={d.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-6 py-3 text-muted-foreground">
                      {new Date(d.dataVencimento).toLocaleDateString("en-US")}
                    </td>
  ...
  return (
    <div className="space-y-6">
      ...
      <Suspense fallback={<div className="animate-pulse h-64 rounded-2xl bg-muted" />}>
        <DespesasContent empresaId={empresaId} />
      </Suspense>
    </div>
  );
}
