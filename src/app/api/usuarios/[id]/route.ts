...
export const GET = withErrorHandler(async (req: Request, context: unknown) => {
  ...
  if (!id)
    return NextResponse.json(
      { error: 'INVALID_ID', message: 'ID inválido', success: false },
      { status: 400 },
    );
  ...
  const nomeCalc =
    found.nomeCompleto ??
    found.nome ??
    (found as unknown as { nome_completo?: string })?.nome_completo ??
    (found as unknown as { full_name?: string })?.full_name ??
    null;
  const normalized = {
    id: found.id,
    email: found.email,
    ...
    expiresAt: found.expiresAt
      ? found.expiresAt instanceof Date
        ...
        : String(found.expiresAt)
      : null,
  };

  // Verificar se há worker vinculado a este usuário
  const workerRecord = await prisma.worker
    .findFirst({
      where: { usuarioId: id },
      ...
      data: {
        ...
        worker: workerRecord
          ? {
              ...
              classification: workerRecord.classification,
            }
          : null,
      },
      success: true,
...
export const PATCH = withErrorHandler(async (req: Request, context: unknown) => {
  const authUser = await requireUser(req);
  const rateCheck = await apiRateLimit.isAllowed(
    req as Parameters<typeof apiRateLimit.isAllowed>[0],
  );
  ...
  if (!isSelfEdit && body.role) {
    ...
    if (!canManageRole(authUser.role as UserRole, newRole as UserRole)) {
      return NextResponse.json(
        {
          error: 'FORBIDDEN',
          message: 'Você não pode promover um usuário a este nível.',
          success: false,
        },
        { status: 403 },
      );
      ...
      if (key === 'senha') {
        ...
        for (const entrada of historico) {
          ...
          if (jaUsada) {
            return NextResponse.json(
              {
                error: 'Senha já utilizada anteriormente. Escolha uma senha diferente.',
                success: false,
              },
              { status: 400 },
            );
        ...
        let column = key;
        if (key === 'nivel' || key === 'role')
          column = cols.has('nivel') ? 'nivel' : cols.has('role') ? 'role' : key;
        if (key === 'zipcode')
          column = cols.has('zipcode') ? 'zipcode' : cols.has('cep') ? 'cep' : key;
        if (cols.has(column)) {
...
export const DELETE = withErrorHandler(
  async (req: Request, context: { params: Promise<{ id: string }> }) => {
    ...
    try {
      ...
      const ip =
        req2.headers?.get('x-forwarded-for') || req2.headers?.get('x-real-ip') || 'unknown';
      await AuditLogger.log({
        userId: Number(authUser.id),
        action: 'DELETE_USER',
        resource: 'Usuario',
