...
export const POST = withErrorHandler(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  ...
  if (!can(user.role as Role, "usuarios", "create")) {
    return NextResponse.json({ error: "Sem permissão para reenviar email de boas-vindas", success: false }, { status: 403 });
  }
  ...
  // Buscar usuário alvo
  type TargetRow = { id: number; email: string; nomeCompleto: string | null; status: string; primeiroAcesso: boolean | number }
  ...
  if (target.status !== "ATIVO") {
    return NextResponse.json({ error: "Não é possível reenviar o email para um usuário inativo", success: false }, { status: 400 });
  }

  const jaConfigurou = target.primeiroAcesso === false || target.primeiroAcesso === 0;
  if (jaConfigurou) {
    return NextResponse.json({ error: "Este usuário já concluiu o primeiro acesso e não precisa do email de boas-vindas", success: false }, { status: 409 });
  }
  ...
  try {
    ...
    logger.error("[resend-welcome] Falha ao enviar email", { targetId }, err);
    return NextResponse.json({ error: "Falha ao enviar o email. Tente novamente.", success: false }, { status: 500 });
  }
  ...
  logger.info("[resend-welcome] Email reenviado", { targetId, targetEmail: target.email, sentBy: user.id });
