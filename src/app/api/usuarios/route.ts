...
function isValidRoleFilter(value: string) {
  return value
    .split(',')
    .map((role) => role.trim())
    .filter(Boolean)
...
export const GET = withErrorHandler(async (req: NextRequest) => {
  ...
  const querySchema = z.object({
    ...
    role: z
      .string()
      .optional()
      .refine((val) => !val || val.trim() === '' || isValidRoleFilter(val), 'role inválido'),
    status: z
      .string()
      .optional()
      .refine((val) => {
    ...
    primeiroAcesso: z
      ...
      .refine(
        ...
        "primeiroAcesso deve ser 'true' ou 'false'",
      ),
    ...
    pageSize: z
      ...
      .refine(
        (v) => !isNaN(v as number) && (v as number) >= 1 && (v as number) <= 100,
        'pageSize inválido',
      ),
  });
  ...
  const effectivePrimeiroAcesso =
    primeiroAcesso === 'true' ? true : primeiroAcesso === 'false' ? false : null;
  ...
  if (effectiveRole) {
    ...
    if (
      ...
      roleValues.some((r) => !canManageRole(userRole, r as UserRole))
    ) {
      return NextResponse.json(
        {
          error: 'Forbidden',
          message: 'Sem permissão para filtrar por este nível.',
          success: false,
        },
        { status: 403 },
      );
...
const UserCreateSchema = z
  .object({
    ...
    workerClassification: z
      .enum(['W2_EMPLOYEE', 'CONTRACTOR_1099', 'SUBCONTRACTOR', 'OWNER_OPERATOR'])
      .optional(),
  })
  .strict();
...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  if (!parsed.success) {
    ...
    for (const error of errors) {
      ...
      if (
        error.message.includes('INVALID_DATE_FORMAT') ||
        error.message.includes('Data de nascimento inválida')
      ) {
        fieldErrors[field] =
  ...
  try {
    ...
    const existsRows = (await withRetry(
      () => prisma.$queryRaw`
    ...
    if (cols.has('status')) {
      ...
      values.push(status ?? 'ATIVO');
    }

    // role/nivel
    if (cols.has('nivel')) {
      insertCols.push('nivel');
      values.push(role ?? 'USUARIO');
    ...
    if (cols.has('nomeCompleto')) {
      insertCols.push('nomeCompleto');
      values.push(nomeCompleto ?? null);
    } else if (cols.has('nome')) {
    ...
    if (cols.has('dataNascimento')) {
      insertCols.push('dataNascimento');
      values.push(dataNascimento ?? null);
    }
    if (cols.has('endereco1')) {
      insertCols.push('endereco1');
      values.push(endereco1 ?? '');
    }
    if (cols.has('endereco2')) {
      insertCols.push('endereco2');
      values.push(endereco2 ?? '');
    }
    if (cols.has('cidade')) {
      insertCols.push('cidade');
      values.push(cidade ?? '');
    }
    if (cols.has('estado')) {
      insertCols.push('estado');
      values.push(estado ?? null);
    }
    if (cols.has('zipcode')) {
      insertCols.push('zipcode');
      values.push(cep ?? null);
    } else if (cols.has('cep')) {
      insertCols.push('cep');
      values.push(cep ?? null);
    }
    if (cols.has('anotacoes')) {
      insertCols.push('anotacoes');
      values.push(anotacoes ?? null);
    }
    if (cols.has('expiresAt') && expiresAtCreate) {
      insertCols.push('expiresAt');
      values.push(expiresAtCreate);
    }

    // Campos para controle de primeiro acesso e senha provisória
    const mustReset = exigirTrocaSenha !== false; // default true
    if (cols.has('primeiroAcesso')) {
      insertCols.push('primeiroAcesso');
      values.push(mustReset);
    }
    if (cols.has('senhaProvisoria')) {
      insertCols.push('senhaProvisoria');
      values.push(mustReset);
    }

    if (cols.has('criadoEm')) {
      insertCols.push('criadoEm');
      values.push(new Date());
    }
    if (cols.has('atualizadoEm')) {
      insertCols.push('atualizadoEm');
      values.push(new Date());
    }
    ...
    const createdRows = (await withRetry(
      () => prisma.$queryRaw`
      ...
      try {
        await prisma.worker.create({
          data: {
            ...
            classification: (workerClassification ?? 'CONTRACTOR_1099') as
              | 'W2_EMPLOYEE'
              | 'CONTRACTOR_1099'
              | 'SUBCONTRACTOR'
              | 'OWNER_OPERATOR',
    ...
    return NextResponse.json(
      {
        ...
        success: true,
        message: workerLinked
          ? 'Usuário criado com sucesso e vinculado ao Workforce'
