...
import { BACKUP_CODE_BCRYPT_ROUNDS } from '@/shared/lib/auth-constants';
...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  // Gerar 8 códigos e hashear em paralelo
  const plainCodes: string[] = Array.from({ length: BACKUP_CODE_COUNT }, generateCode);
  const hashes = await Promise.all(
    plainCodes.map((code) => bcrypt.hash(code, BACKUP_CODE_BCRYPT_ROUNDS)),
  );
