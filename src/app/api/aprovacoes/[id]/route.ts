  ...
  async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    if (parsed.type === 'expense') {
      const ea = await prisma.expenseApproval.findUnique({
        // nosemgrep: semgrep.gladpros.gladpros-empresaId-required-on-prisma-where
      ...
      if (!ea)
        return NextResponse.json(
          { error: 'Aprovação não encontrada', success: false },
          { status: 404 },
        );
    ...
    if (!proposta)
      return NextResponse.json(
        { error: 'Proposta não encontrada', success: false },
        { status: 404 },
      );
...
export const PUT = withErrorHandler(
  async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    if (!can(user.role as Role, 'aprovacoes', 'update')) {
      return NextResponse.json(
        { error: 'Forbidden', message: 'Sem permissão para aprovar/rejeitar', success: false },
        { status: 403 },
      );
    }
    ...
    if (!body.success) {
      return NextResponse.json(
        { error: 'Dados inválidos', message: body.error.issues[0]?.message, success: false },
        { status: 400 },
      );
    }
    ...
    if (acao === 'rejeitar' && !comentario) {
      return NextResponse.json(
        { error: 'Comentário obrigatório para rejeição', success: false },
        { status: 400 },
      );
    }

    if (parsed.type === 'expense') {
      const ea = await prisma.expenseApproval.findUnique({ where: { id: parsed.numericId } }); // nosemgrep: semgrep.gladpros.gladpros-empresaId-required-on-prisma-where
      if (!ea)
        return NextResponse.json(
          { error: 'Aprovação não encontrada', success: false },
          { status: 404 },
      ...
      if (ea.status !== 'PENDENTE' && ea.status !== 'EM_ANALISE') {
        return NextResponse.json(
          { error: 'Esta aprovação já foi processada', success: false },
          { status: 409 },
        );
      }
