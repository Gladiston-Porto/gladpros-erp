...
export const POST = withErrorHandler(
  async (request: Request, { params }: { params: Promise<{ id: string }> }) => {
    ...
    if (
      !can(user.role as Role, 'service-orders', 'update') ||
      !can(user.role as Role, 'financeiro', 'create')
    ) {
    ...
    if (!attachment) {
      return NextResponse.json(
        {
          error: 'Not found',
          message: 'Anexo não encontrado nesta ordem de serviço',
          success: false,
        },
        { status: 404 },
      );
    ...
    if (attachment.type !== 'RECEIPT') {
      return NextResponse.json(
        {
          error: 'Bad request',
          message: 'Apenas anexos do tipo RECEIPT podem gerar reembolso',
          success: false,
        },
        { status: 400 },
      );
    ...
    let category = await prisma.expenseCategory.findFirst({
      where: {
        empresaId,
        OR: [{ nome: { contains: 'Reimbolso' } }, { nome: { contains: 'Reemb' } }],
      },
      select: { id: true },
    });
