...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (requestedRole !== 'all' && !(requestedRole in roleAliases)) {
    return NextResponse.json(
      { error: 'Validation failed', message: 'Parâmetro "role" inválido', success: false },
      { status: 400 },
    );
  }

  const normalizedRole =
    requestedRole === 'all' ? 'all' : roleAliases[requestedRole as keyof typeof roleAliases];
  const roleResult = z
    .enum(['all', 'ADMIN', 'GERENTE', 'USUARIO', 'FINANCEIRO', 'ESTOQUE', 'CLIENTE'])
    .safeParse(normalizedRole);
  const safeRole = roleResult.success ? roleResult.data : 'all';

  const cacheKey = `dashboard_analytics:${user.empresaId}:${String(user.role).toUpperCase()}:${canReadAnalytics ? 'analytics' : 'dashboard'}:${safeRole}:${period}`;
  ...
  const analyticsData = await withBusinessCache(
    ...
    async () => {
      ...
      const roleFilter =
        safeRole !== 'all' && validRoles.includes(safeRole as Role)
          ? { empresaId: user.empresaId, nivel: safeRole }
      ...
      const [
        ...
        >`
            ...
            FROM Proposta
                WHERE criadoEm >= ${sixMonthsAgo} AND empresaId = ${user.empresaId} AND deletedAt IS NULL
        ...
        canReadAnalytics
          ? prisma.$queryRaw<
            ...
            >`
                ...
                FROM Usuario
                  WHERE criadoEm >= ${sixMonthsAgo} AND empresaId = ${user.empresaId}
                GROUP BY DATE_FORMAT(criadoEm, '%Y-%m'), DATE_FORMAT(criadoEm, '%b')
                ORDER BY month ASC
              `
          : Promise.resolve([]),
        canReadAnalytics
          ? prisma.$queryRaw<
            ...
            >`
                ...
                FROM Usuario
                WHERE ultimoLoginEm >= ${sixMonthsAgo} AND empresaId = ${user.empresaId}
      ...
      const metricsMap: Record<
        string,
        { name: string; usuarios: number; ativos: number; propostas: number }
      > = {};
      for (const row of monthlyProposals) {
        if (!metricsMap[row.month])
          metricsMap[row.month] = { name: row.label, usuarios: 0, ativos: 0, propostas: 0 };
      ...
      const propostasAprovadas =
        (statusCountMap['APROVADA'] || 0) + (statusCountMap['ASSINADA'] || 0);
      const propostasPendentes =
        (statusCountMap['RASCUNHO'] || 0) + (statusCountMap['ENVIADA'] || 0);
      ...
      const systemHealth: 'good' | 'warning' | 'error' = !canReadAnalytics
        ? 'warning'
      ...
      return {
        ...
        charts: {
          loginAttemptsByDay: canReadAnalytics
            ? loginAttemptsByDay.map((row) => ({
                date: row.date,
            ...
            : [],
          auditActions: canReadAnalytics
            ? auditActionsByType.map((row) => ({
                action: row.acao,
                count: row._count.acao,
              }))
            : [],
          usersByRole: canReadAnalytics
            ? usersByNivel.map((row) => ({
                role: row.nivel,
