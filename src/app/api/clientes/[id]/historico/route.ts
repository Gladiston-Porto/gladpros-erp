...
export const GET = withErrorHandler(
  async (request: NextRequest, ctx: { params: Promise<{ id: string }> }) => {
    ...
    const outstandingInvoiceValue = invoices
      ...
      .reduce((sum, inv) => sum + Number(inv.valorTotal), 0);
    const projectPipelineValue = projetos.reduce(
      (sum, proj) => sum + Number(proj.valorEstimado ?? 0),
      0,
    );
    const activeWarrantyTickets = warrantyTickets.filter(
      (ticket) => ticket.status !== 'RESOLVED' && ticket.status !== 'DENIED',
    ).length;
    ...
    const outstandingRevenueValue = revenues
      ...
      .reduce((sum, revenue) => sum + Number(revenue.valor), 0);
    const invoicePaymentsValue = invoicePayments.reduce(
