  ...
  ctx: { params: Promise<{ id: string }> }) => {
    ...
    if (!rlPut.allowed) {
      return NextResponse.json(
        ...
        { status: 429, headers: { 'X-RateLimit-Remaining': rlPut.remaining.toString(), 'X-RateLimit-Reset': rlPut.resetTime.toString() } }
      )
    }
    ...
    const updateableFields = [
      ...
      'endereco', 'observacoes', 'tipoDocumentoPF',
    ...
    if (isUpdatingAddress) {
      ...
      const mergedAddress = {
        addressStreet: updateData['addressStreet'] !== undefined ? updateData['addressStreet'] as string : clienteAtual.addressStreet,
        addressCity: updateData['addressCity'] !== undefined ? updateData['addressCity'] as string : clienteAtual.addressCity,
        addressState: updateData['addressState'] !== undefined ? updateData['addressState'] as string : clienteAtual.addressState,
        addressZip: updateData['addressZip'] !== undefined ? updateData['addressZip'] as string : clienteAtual.addressZip
      }
    ...
    const mergedNomeCompleto = updateData.nomeCompleto !== undefined ? updateData.nomeCompleto : clienteAtual.nomeCompleto
    const mergedNomeFantasia = updateData.nomeFantasia !== undefined ? updateData.nomeFantasia : clienteAtual.nomeFantasia
    const mergedRazaoSocial = updateData.razaoSocial !== undefined ? updateData.razaoSocial : clienteAtual.razaoSocial

    updateData.nomeChave = (mergedNomeCompleto as string) || (mergedNomeFantasia as string) || (mergedRazaoSocial as string) || ''
    ...
    if (sanitizedData.email !== undefined && sanitizedData.email !== clienteAtual.email) {
      ...
      if (conflicting) {
        const message = conflicting.status === 'INATIVO'
        ...
        return NextResponse.json(
          { error: 'Conflict', message, reactivable: conflicting.status === 'INATIVO', conflictingId: conflicting.id, success: false },
          { status: 409 }
        )
      }
    }
  ...
  ctx: { params: Promise<{ id: string }> }) => {
    ...
    if (!rlDel.allowed) {
      return NextResponse.json(
        ...
        { status: 429, headers: { 'X-RateLimit-Remaining': rlDel.remaining.toString(), 'X-RateLimit-Reset': rlDel.resetTime.toString() } }
      )
    }
