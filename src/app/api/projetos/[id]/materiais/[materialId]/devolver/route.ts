...
// Schema de validação para devolução de material
const devolverMaterialSchema = z.object({
...
export const POST = withErrorHandler(async (request: NextRequest,
  context: { params: Promise<{ id: string; materialId: string }> }) => {
    ...
    const { quantidade, observacao } = devolverMaterialSchema.parse(body)
    
    // Executa devolução
    const service = new InventoryMovementService()
    const movimentacao = await service.devolverMaterial({
      projetoId,
      materialId,
      quantidade,
