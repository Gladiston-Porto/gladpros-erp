...
export function ReceberCompraForm({ compra, localizacoes }: ReceberCompraFormProps) {
    ...
    const defaultValues: ReceberCompraFormData = {
        ...
        itens: compra.itens.map(item => ({
            ...
            quantidadeRecebida: Math.max(0, Number(item.quantidade) - Number(item.quantidadeEntregue ?? 0)),
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                                            <div>
                                                ...
                                                <div className="flex gap-4 text-sm text-gray-600 mt-1">
                                                    <span>Pedido: <span className="font-semibold">{qtdPedida} {field.unidade}</span></span>
                                                    <span>Já Entregue: <span className="font-semibold">{qtdEntregue} {field.unidade}</span></span>
                                                    <span className={saldoPendente > 0 ? "text-orange-600 font-medium" : "text-green-600 font-medium"}>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                                            ...
                                            <div className="md:col-span-3">
                                                ...
                                                {errors.itens?.[index]?.quantidadeRecebida && (
                                                    <p className="text-xs text-red-500 mt-1">{errors.itens[index]?.quantidadeRecebida?.message}</p>
                                                )}
                                            </div>
                                            ...
                                            <div className="md:col-span-5">
                                                ...
                                                {errors.itens?.[index]?.localizacaoId && (
                                                    <p className="text-xs text-red-500 mt-1">{errors.itens[index]?.localizacaoId?.message}</p>
                                                )}
                                            </div>
