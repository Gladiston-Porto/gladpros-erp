...
export const DEFAULT_NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        href: '/dashboard/financeiro/transferencias',
  ...
  },
  {
    ...
    items: [
      ...
      {
        href: '/dashboard/financeiro/fiscal/impostos-estimados',
        label: 'Impostos Estimados',
        icon: Bell,
      },
      { href: '/dashboard/financeiro/payables', label: 'Payables (1099)', icon: HandCoins },
      {
        href: '/dashboard/financeiro/fiscal/compensacao',
        label: 'Compensação do Owner',
        icon: Wallet,
      },
      {
      ...
      {
        href: '/dashboard/financeiro/fiscal/categorias',
        label: 'Categorias Fiscais',
        icon: ClipboardList,
      },
    ],
  },
  {
    title: 'SISTEMA',
    items: [
      { href: '/usuarios', label: 'Usuários', icon: ShieldCheck, requiredRoles: ['ADMIN'] },
...
const ROLE_THEME: Record<UserRole, { chip: string; glow: string }> = {
  ...
  GERENTE: {
    ...
    glow: 'shadow-[0_0_0_3px_rgba(255,140,0,0.2)]',
  },
...
export default function DashboardShell({
  ...
  const filteredGroups = useMemo(
    () => filterNavGroupsByRole(groups, (user?.role || 'USUARIO') as Role),
    [groups, user?.role],
  );
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-background" data-testid="shell-root">
      ...
      {/* CONTENT */}
      <div
        className={`${collapsed ? 'pl-[84px]' : 'pl-[280px]'} pt-16 transition-all duration-300`}
      >
...
export function HeaderBar({
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-xl backdrop-saturate-150 dark:bg-background/70">
      <div
        className={`mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-3 px-4 ${collapsed ? 'pl-24' : 'pl-72'} transition-all duration-300`}
      >
        <div className="flex items-center gap-2">
          <span className="font-display text-base opacity-70">GladPros</span>
          <span
            className={`ml-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${roleTheme.chip}`}
          >
            {user.role}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <button
            className="relative rounded-xl p-2 transition-colors duration-150 hover:bg-accent"
            aria-label="Alertas"
          >
            <Bell className="h-5 w-5" />
            <span className="absolute -right-0.5 -top-0.5 grid h-4 w-4 place-content-center rounded-full bg-brand-secondary text-[10px] font-medium text-white animate-pulse-soft">
              3
            </span>
          </button>
          <button
            onClick={onToggleTheme}
            className="rounded-xl p-2 transition-colors duration-150 hover:bg-accent"
          ...
          <div
            className={`ml-1 hidden items-center gap-2 rounded-xl border border-border bg-card px-2.5 py-1.5 md:flex ${roleTheme.glow}`}
          >
            <Image
...
export function Sidebar({
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-40 h-screen ${collapsed ? 'w-[84px]' : 'w-[280px]'} transition-all duration-300`}
      aria-label="Sidebar"
      data-testid="shell-sidebar"
    >
      <div className="relative flex h-full flex-col border-r border-white/10 bg-hero-gradient text-white dark:border-white/5">
        <div className="flex items-center justify-between gap-2 px-4 py-4">
          ...
          <button
            onClick={onToggle}
            className="rounded-xl p-2 hover:bg-white/10 transition-colors duration-150"
              ...
              {group.items.map((item, idx) => (
                <SidebarItem
                  ...
                  active={
                    ...
                    (activeHref?.startsWith(item.href) && item.href !== '/')
                  }
                  collapsed={collapsed}
                  delay={idx * 0.03}
                />
        ...
        <div className="border-t border-white/10 px-3 pb-4 pt-3">
          ...
          {!collapsed && (
            <div className="mt-2 text-center text-[9px] text-white/25 tracking-widest">
              v3.0 · GladPros
            </div>
          )}
        </div>
...
function SidebarItem({
  ...
  return (
    <Link
      href={href}
      className="relative block focus:outline-none focus-visible:ring-2 focus-visible:ring-white/30 rounded-xl"
    >
      {/* Active accent bar */}
      {active && (
        <span
...
export function Panel({
  ...
  return (
    <div
      className={`rounded-xl border border-border bg-card p-4 sm:p-5 shadow-card transition-shadow duration-200 hover:shadow-card-hover ${className}`}
      data-testid="panel"
    >
