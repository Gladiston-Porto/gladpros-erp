...
export class SecurityService {
  ...
  static async createSession(
    ...
    // Cleanup de sessões expiradas (throttled) e revogação das sessões do usuário
    // em paralelo para não penalty um ao outro.
    const cleanupPromise = (Date.now() - lastSessionCleanupAt >= SESSION_CLEANUP_INTERVAL)
      ? this.cleanExpiredSessions()
          .then(() => { lastSessionCleanupAt = Date.now(); })
          .catch(() => { /* ok se tabela ainda não existir */ })
      : Promise.resolve();

    await Promise.all([
      cleanupPromise,
      this.revokeAllUserSessions(usuarioId).catch(() => { /* continuar mesmo se falhar */ })
    ]);
