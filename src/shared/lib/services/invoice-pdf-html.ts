/**
 * Invoice PDF Generation via Playwright
 *
 * Renders the print page (/invoices/[id]/print) in a headless browser
...
/**
 * Generates a PDF buffer by rendering the invoice print page.
 */
export async function generateInvoicePDFFromHTML(
  options: GenerateOptions,
): Promise<Buffer> {
  const { invoiceId, baseUrl, cookie } = options;
  const url = `${baseUrl}/invoices/${invoiceId}/print`;
