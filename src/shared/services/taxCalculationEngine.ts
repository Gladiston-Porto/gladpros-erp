...
export async function calculateTax(
  ...
  let taxableForIncomeTax = netIncome
  if (regime === "LLC_DEFAULT") {
    // Deduct 50% of SE tax from income for income tax calculation
    taxableForIncomeTax = netIncome - selfEmploymentTax * SE_DEDUCTION_FACTOR
  } else {
    // S-Corp: owner salary already deducted as expense; distributions not deductible
    // Income tax is on the K-1 income (net income after salary)
    taxableForIncomeTax = netIncome - ownerSalaryYTD
  }
