...
export async function calculateTax(input: TaxCalculationInput): Promise<TaxCalculationResult> {
  ...
  const expensesByScheduleCLine: ScheduleCLineItem[] = Array.from(lineMap.entries())
    .map(([lineNumber, data]) => ({
      lineNumber,
  ...
  const selfEmploymentTax = regime === 'LLC_DEFAULT' ? calculateSelfEmploymentTax(netIncome) : 0;
  ...
  const estimatedIncomeTax = calculateFederalIncomeTax(Math.max(0, taxableForIncomeTax));

  // 8. Total estimated tax
  const totalEstimatedTax = Math.round((estimatedIncomeTax + selfEmploymentTax) * 100) / 100;

  const quarterlyPaymentTarget = Math.round((totalEstimatedTax / 4) * 100) / 100;
...
export async function calculateYTDTax(empresaId: number): Promise<TaxCalculationResult> {
...
export async function calculateAnnualizedEstimate(
  ...
  return {
    ...
    totalDeductibleExpenses:
      Math.round(ytd.totalDeductibleExpenses * annualizationFactor * 100) / 100,
    ...
    quarterlyPaymentTarget:
      Math.round(((ytd.totalEstimatedTax * annualizationFactor) / 4) * 100) / 100,
    annualizationFactor,
  };
}
