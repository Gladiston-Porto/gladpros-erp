  ...
  try {
    const payment = await prisma.$transaction(async (tx) => {
      ...
      if (movementAmount.lte(0)) {
        throw new Error(
          'Novo pagamento deve aumentar o valor pago; use ajuste/estorno para reduzir',
        );
      }
      ...
      await postLedgerTransaction(
        {
          ...
          entries: [
            ...
            {
              accountCode: 'CASH',
              credit: movementAmount,
              memo: 'Saída de caixa para estimated tax',
            },
          ],
        },
        tx,
    ...
    return {
      ...
      error:
        error instanceof Error ? error.message : 'Erro ao registrar pagamento de estimated tax',
    };
  }
}
