 ...
 */
export async function getOrCreateAssignmentDefault(params: GetOrCreateAssignmentParams) {
...
export async function submitTimesheet(timesheetId: number, submittedById: number) {
  ...
  if (timesheet.status !== TimesheetStatus.DRAFT) {
    throw new Error(
      `Timesheet precisa estar em DRAFT para ser submetido. Status atual: ${timesheet.status}`,
    );
  }
...
export async function approveTimesheet(timesheetId: number, approvedById: number) {
  ...
  if (timesheet.status !== TimesheetStatus.SUBMITTED) {
    throw new Error(
      `Timesheet precisa estar SUBMITTED para ser aprovado. Status atual: ${timesheet.status}`,
    );
  }

  // Calculate totals
  const totalHours = timesheet.entries.reduce((sum, entry) => sum + Number(entry.hours), 0);
...
export async function approveMilestone(milestoneId: number, approvedById: number) {
  ...
  if (milestone.status !== MilestoneStatus.SUBMITTED) {
    throw new Error(
      `Milestone precisa estar SUBMITTED para ser aprovado. Status atual: ${milestone.status}`,
    );
  }
...
export async function generatePayable(params: GeneratePayableParams) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // 3. Calculate total
    const timesheetTotal = timesheets.reduce((sum, t) => sum + Number(t.totalCost), 0);
...
export async function markPayableAsPaid(params: MarkPayableAsPaidParams) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (payable.status !== PayableStatus.APPROVED) {
      throw new Error(
        `Payable precisa estar APPROVED para ser pago. Status atual: ${payable.status}`,
      );
    }
