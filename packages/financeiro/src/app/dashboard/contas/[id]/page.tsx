...
export default function BankAccountDetailsPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    loadData();
  }, [params.id, tipoFilter, dataInicio, dataFim]);
  ...
  async function loadData() {
    ...
    try {
      const [contaRes, extratoRes] = await Promise.all([
        ...
        fetch(`/api/financeiro/contas/${params.id}/extrato?${new URLSearchParams({
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          <div>
            ...
            <p className="text-2xl font-bold">R$ {Number(conta.saldoAtual).toFixed(2)}</p>
          </div>
          <div>
            <p className="text-sm text-gray-600">Créditos</p>
            <p className="text-xl text-green-600 flex items-center gap-1">
              <TrendingUp className="w-5 h-5" />
              R$ {extrato.resumo.totalCreditos.toFixed(2)}
            </p>
          </div>
          <div>
            ...
            <p className="text-xl text-red-600 flex items-center gap-1">
              ...
              R$ {extrato.resumo.totalDebitos.toFixed(2)}
            </p>
          </div>
        </div>
      </div>
              ...
              {extrato.transacoes.map((t: BankTransaction) => (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-right font-medium text-gray-900">
                    R$ {Number(t.saldoPosterior).toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
