  ...
  try {
    ...
    if (saldoAtualTotal < 0) {
      alertas.push({
        ...
        mensagem: `Saldo total negativo: R$ ${saldoAtualTotal.toFixed(2)}`,
    ...
    if (despesasVencidas.length > 0) {
      ...
      alertas.push({
        ...
        mensagem: `${despesasVencidas.length} despesa(s) vencida(s) - Total: R$ ${totalVencido.toFixed(2)}`,
    ...
    if (receitasAtrasadas.length > 0) {
      ...
      alertas.push({
        ...
        mensagem: `${receitasAtrasadas.length} receita(s) atrasada(s) - Total: R$ ${totalAtrasado.toFixed(2)}`,
    ...
    if (projecoes && projecoes.totaisProjetados.saldoFinal < 0) {
      alertas.push({
        ...
        mensagem: `Projeção de saldo negativo em ${diasProjecao} dias: R$ ${projecoes.totaisProjetados.saldoFinal.toFixed(2)}`,
        diasProjecao,
    ...
    return NextResponse.json({
      success: false,
      message: "Erro ao gerar análise de fluxo de caixa",
