  ...
  return successResponse({
    ...
    estatisticas: {
      totalAtivos: stats.reduce((acc, s) => {
        const count = typeof s._count === 'number' ? s._count : ((s._count as Record<string, unknown>)?._all as number || 0);
        return acc + count;
      }, 0),
      porTipo: stats.reduce((acc: Record<string, number>, s) => {
        const count = typeof s._count === 'number' ? s._count : ((s._count as Record<string, unknown>)?._all as number || 0);
        acc[s.tipo] = (acc[s.tipo] || 0) + count;
        return acc;
      }, {}),
      porPrioridade: stats.reduce((acc: Record<string, number>, s) => {
        const count = typeof s._count === 'number' ? s._count : ((s._count as Record<string, unknown>)?._all as number || 0);
